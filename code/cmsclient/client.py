...
class CompletionCache(object):
    """The completion cache is how we support tab-completion with cmsclient.

    The `Manager` writes object IDs and Human-IDs to the completion-cache on
    object-show, object-list, and object-create calls.

    The `cms.bash_completion` script then uses these files to provide the
    ...
    def _make_directory_name(self, username, auth_url):
        ...
        base_dir = utils.env('cmsCLIENT_UUID_CACHE_DIR',
                             default="~/.cmsclient")
...
class SessionClient(adapter.LegacyJsonAdapter):
    ...
    def request(self, url, method, **kwargs):
        ...
        # keystoneclient, where we need to raise the cmsclient errors.
...
class HTTPClient(object):
    ...
    def _cs_request(self, url, method, **kwargs):
        ...
        if url is None:
            ...
            # a cms endpoint directly without "v2/<tenant-id>".
    ...
    def authenticate(self):
        ...
        if self.version == "v2.0":  # FIXME(chris): This should be better.
            ...
            # In some configurations cms makes redirection to
        ...
        if self.bypass_url:
            ...
            raise exceptions.Unauthorized('cms Client')
...
def _construct_http_client(username=None, password=None, project_id=None,
                           ...
                           auth=None, user_agent='python-cmsclient',
                           **kwargs):
    if session:
...
def get_client_class(version):
    version_map = {
        '1.0': 'cmsclient.v1_0.client.Client',
    }
    try:
