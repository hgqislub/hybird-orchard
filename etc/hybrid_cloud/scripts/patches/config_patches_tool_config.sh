#!/bin/sh
if [ $# != 6 ]; then
    echo "Usage: sh $0 proxy_name proxy_host_name az_domain remote_data_subnet remote_data_gw cascading_domain"
    exit 1
fi
...
cascaded_add_route=${openstack_data_subnet}":"${vcloud_data_gw}
